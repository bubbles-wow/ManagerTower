...
from src.modules import METERTransformerSS, BTTransformer, MTTransformer
...
def main(_config):
    ...
    elif _config["model_type"] == "MT":
        model = MTTransformer(_config)
    else:
    ...
    if not _config["test_only"]:
        ...
        if _config["group_name"] not in ["mlm_itm", "nlvr2", "snli", "irtr_itm_itc_f30k", "irtr_itm_itc_coco"]: # these tasks do not need to run the test evaluation after training.
            ...
            elif test_config["model_type"] == "MT":
                test_model = MTTransformer(test_config)
